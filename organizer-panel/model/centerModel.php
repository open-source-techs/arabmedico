...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
	if(isset($_POST['action']) && $_POST['action'] == "getUserList")
	{
		...
		{
			$data = array();
			$sql = query("SELECT * FROM tbl_organizer WHERE org_id != ".get_sess("userdata")['org_id']);
			if(nrows($sql) > 0)
			{
				$i = 0;
				while($sqlData = fetch($sql))
	...
	else if(isset($_POST['txt_action']))
    {
        ...
        $data['sender']       	= get_sess("userdata")['org_id'];
        $data['sender_type']  	= "organizer";
    ...
    else if(isset($_POST['fetch_list']))
    {
        ...
        $doctor_id = get_sess("userdata")['org_id'];
    ...
    {
    	$receiver = get_sess("userdata")['org_id'];
...
else if($_SERVER['REQUEST_METHOD'] == "GET")
{
	...
	{
		$data['my_id'] 			= get_sess("userdata")['org_id'];
		$data['my_type'] 		= 'organizer';
		...
		if(insert($data,'tbl_user_contact'))
		{
            ...
            $myName = get_sess("userdata")['org_name'];
			$myImg 	= get_sess("userdata")['org_icon'];
			$message = '<li>
                <div class="border-gray">
                    <div class="pull-left">
                        <img src="'.file_url().$myImg.'" class="img-thumbnail" alt="Organizer Image">
                    </div>
                    <h4>'.$myName.' (Organizer)</h4>
