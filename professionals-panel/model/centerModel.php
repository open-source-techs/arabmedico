...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
	if(isset($_POST['action']) && $_POST['action'] == "getUserList")
	{
		...
		{
			$data = array();
			$sql = query("SELECT * FROM tbl_candidate WHERE candidate_id != ".get_sess("userdata")['candidate_id']);
			if(nrows($sql) > 0)
			{
				$i = 0;
				while($sqlData = fetch($sql))
    ...
    else if(isset($_POST['fetch_list']))
    {
        ...
        $doctor_id = get_sess("userdata")['candidate_id'];
    ...
    {
    	$receiver = get_sess("userdata")['candidate_id'];
...
else if($_SERVER['REQUEST_METHOD'] == "GET")
{
	...
	{
		$data['my_id'] 			= get_sess("userdata")['candidate_id'];
		$data['my_type'] 		= 'professional';
		...
		if(insert($data,'tbl_user_contact'))
		{
			...
			$myImg 	= get_sess("userdata")['candidate_image'];
			$message = '<li>
                <div class="border-gray">
                    <div class="pull-left">
                        <img src="'.file_url().$myImg.'" class="img-thumbnail" alt="professional Image">
                    </div>
                    <h4><a href="'.base_url().$mySlug.'" target="_blank">'.$myName.'</a> (professional)</h4>
