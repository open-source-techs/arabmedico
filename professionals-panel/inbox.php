                    ...
                    <div class="card">
                        <div class="chat_list">
                            <ul class="user_list list-unstyled mb-0 mt-3 myclass">
                                ...
                                $doctorID = get_sess("userdata")['candidate_id'];
                                $senderlist =array();
                                $sendersql = query("SELECT DISTINCT(c.sender), c.sender_type FROM tbl_chat c WHERE c.receiver = $doctorID AND receiver_type = 'professional'");
                                ...
                                if(isset($senderlist[0]))
                                {
                                    ...
                                    $sendersql = query("SELECT DISTINCT(c.receiver), c.receiver_type FROM tbl_chat c WHERE c.sender_type = 'professional' AND c.sender = $doctorID AND c.receiver NOT IN ($send_list) ");
                                }
                                else
                                {
                                    $sendersql = query("SELECT DISTINCT(c.receiver), c.receiver_type FROM tbl_chat c WHERE c.sender_type = 'professional' AND c.sender = $doctorID");
                                }
                            ...
                            if(isset($_GET['IdChat']))
                            {
                                ...
                                <ul class="chat-history">
                                    ...
                                    while ($msg = fetch($msgsql))
                                    {
                                        ...
                                        query("UPDATE `tbl_chat` SET chat_read = 1 WHERE sender = $senderID AND receiver = $doctorID AND receiver_type = 'professional' AND sender_type = '$sender_type'");
                                    }
                                    $msgsql = query("SELECT * FROM tbl_chat WHERE sender = $doctorID AND receiver = $senderID AND receiver_type = '$sender_type' AND sender_type = 'professional' ORDER BY chat_id ASC");
                                    while ($msg = fetch($msgsql))
                                    {
                                        $receiverId = $msg['receiver'];
                                        ?>
                                        ...
                                        <?php
                                        query("UPDATE `tbl_chat` SET chat_read = 1 WHERE sender = $doctorID AND receiver = $senderID AND receiver_type = '$sender_type' AND sender_type = 'professional'");
                                    }
                                    ?>
                                </ul>
                                <div class="col-md-12">
