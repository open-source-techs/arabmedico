...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
	...
	else if(isset($_POST['action']) && $_POST['action'] == "change_status")
	{
	   ...
	   // die();
	    if($value == "deactive")
	    {
	        $sql = query("UPDATE tbl_slider SET slider_video_show = 0 ");
	        if($sql)
	        {
	            $arr = array(
	                "status" => "done",
	    ...
	    {
	        $data['slider_video_show'] = 0;
	        where('slider_id !',"0");
	        $sql = update($data,"tbl_slider");
	        if($sql)
	        {
	            $data1['slider_video_show'] = 1;
	            where('slider_id',$slide);
	            if(update($data1, 'tbl_slider'))
    			{
    				$arr = array(
    	                "status" => "done",
