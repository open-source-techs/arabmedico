			...
			<div class="col-sm-12">
				<div class="panel panel-bd lobidrag">
                    <div class="panel-heading">
                        <div class="btn-group"> 
                            <a class="btn btn-success" href="<?= admin_base_url();?>add-cme"> <i class="fa fa-plus"></i> Add CME
                            </a>
                        </div>
                    </div>
                    <div class="panel-body">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-hover">
                                ...
                                <tbody>
                                    <?php
                                    $sql = query("SELECT * FROM tbl_cme c LEFT JOIN tbl_organizer o ON (c.cme_organizer = o.org_id)");
                                    while ($row = fetch($sql))
                                    {
                                        ?>
                                        <tr>
                                            <td><img src="<?= file_url().$row['cme_icon'];?>" class="img-fluid" style="width:auto;height:50px"></td>
                                            <td><?= $row['cme_topic'];?></td>
                                            <td><?= $row['cme_depart'];?></td>
                                            <td><?= $row['cme_credits'];?></td>
                                            <td><?= $row['org_name'];?></td>
                                            <td><?= $row['close_date'];?></td>
                                            <td>
                                                <a href="<?= admin_base_url();?>edit-cme?cme_id=<?= $row['id']; ?>" class="btn btn-info btn-xs"><i class="fa fa-pencil"></i>
                                                </a>
                                                <a href="<?= admin_base_url();?>model/cmeModel?act=del&cme_id=<?= $row['id']; ?>" class="btn btn-danger btn-xs"><i class="fa fa-trash-o"></i>
                                                </a>
                                            </td>
                                        </tr>
                                        <?php
