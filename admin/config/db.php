<?php 
	...
	{
		return "https://arabmedico.com/admin/";
		// return "http://localhost/medical/admin/";
	}
...
// 			die();
	...
	function checkUniqueCol($table,$col,$value,$where=false,$emailCol='',$email='')
	{
		...
		$det_res = mysqli_fetch_array($res);
		if($det_res['count'] > 0 )
		{
		    return false;
		}
	...
	{
        $sql = query("SELECT * FROM tbl_language ORDER BY lang_id ASC");
        $arr = array();
        $i = 0;
        while($cnt = fetch($sql))
	    {
