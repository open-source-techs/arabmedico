			...
			<div class="col-sm-12">
				<div class="panel panel-bd lobidrag">
                    <div class="panel-heading">
                        <div class="btn-group"> 
                            <a class="btn btn-success" href="<?= admin_base_url();?>add-dpt-doctor?dpt_id=<?= $dpt_id; ?>"> <i class="fa fa-plus"></i> Add Doctor
                            </a>
                        </div>
                    </div>
                    <div class="panel-body">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-hover">
                                ...
                                <tbody>
                                    <?php
                                    // echo "SELECT * FROM tbl_doctor d JOIN tbl_department td ON (td.dpt_id = d.doctor_department) WHERE 1=1 $where LIMIT $offset, $limit";
                                    $sql = query("SELECT * FROM tbl_doctor d JOIN tbl_department td ON (td.dpt_id = d.doctor_department) WHERE doctor_department = $dpt_id $where LIMIT $offset, $limit");
                                    while ($row = fetch($sql))
                                    {
                                        ?>
                                        <tr>
                                            ...
                                            <td><?= $row['doc_job_title'];?></td>
                                            <td><?= ($row['doc_active'] == 1) ? 'Active' : "Not Active";?></td>
                                            <td><?= date("d/m/Y",strtotime($row['doc_created_at']));?></td>
                                            <td>
                                                <a href="<?= admin_base_url();?>edit-dpt-doc?dpt_id=<?= $dpt_id; ?>&doc_id=<?= $row['doc_id']; ?>" class="btn btn-info btn-xs"><i class="fa fa-pencil"></i>
                                                </a>
                                                <a href="<?= admin_base_url();?>model/doctorModel?act=del&doc_id=<?= $row['doc_id']; ?>&dpt_id=<?= $dpt_id; ?>" class="btn btn-danger btn-xs"><i class="fa fa-trash-o"></i>
                                                </a>
                                            </td>
                                        </tr>
                                        <?php
