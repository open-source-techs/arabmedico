                ...
                <div class="panel panel-bd lobidrag">
                    ...
                    <div class="panel-body">
                        <form  action="<?= admin_base_url()?>model/jobModel" method="POST" enctype="multipart/form-data" class="col-sm-12">
                            ...
                            <div class="col-sm-6 form-group">
                                <label>Select Employeer</label>
                                <select class="form-control" name="txt_employeer">
                                    <option value="0">No Employeer</option>
                                    <?php
                                    $sql_emp = query("SELECT * FROM tbl_employer");
                                    while($emp = fetch($sql_emp))
                                    {
                                        ?>
                                        <option <?= ($emp['emp_id'] == $job['job_employeer']) ? 'selected' : ''; ?> value="<?= $emp['emp_id'];?>"><?= $emp['emp_name'];?> - <?= $emp['emp_name_ar'];?></option>
                                        <?php
                                    }
                                    ?>
                                </select>
