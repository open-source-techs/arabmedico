...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
	if(isset($_POST['btn_save_slide']))
	{
	    ...
	    $data['slider_hospital']         = post('txt_hospital_id');
	    if($data['slider_is_video'] == 1)
	    {
	       ...
	       // print_r($data);
	       // die();
	        if(insert2($data, 'tbl_hospital_slider'))
			{
	...
	else if(isset($_POST['btn_edit_slide']))
	{
	    ...
	    if($data['slider_is_video'] == 1)
	    {
    		...
    		if(update2($data, 'tbl_hospital_slider'))
			{
				set_msg('Success','Slide is added successfully','success');
				jump(admin_base_url()."slider");
			}
	    ...
	    else
	    {
    		...
    		{
    			where('slider_id',$slider_id);
    			if(update($data, 'tbl_hospital_slider'))
    			{
	...
	else if(isset($_POST['action']) && $_POST['action'] == "change_status")
	{
	    $value = post('val');
	    $slide = post('sl_id');
	    $hospitalId = post('hospital');
	    if($value == "deactive")
	    {
	        $sql = query("UPDATE tbl_hospital_slider SET slider_video_show = 0 WHERE slider_hospital = $hospitalId");
	        if($sql)
	        {
	            $arr = array(
	                "status" => "done",
	    ...
	    else if($value == "active")
	    {
	        ...
	        if($sql)
	        {
	            ...
	            if(update($data1, 'tbl_hospital_slider'))
    			{
    				$arr = array(
    	                "status" => "done",
