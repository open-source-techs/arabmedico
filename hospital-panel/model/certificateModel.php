...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
	...
	{
	    $data['certificate_hospital']         = post('txt_hospitalID');
		$data['certificate_title'] 			= post('txt_cer_name');
		$data['certificate_title_arabic'] 	= $_POST['arabic_cer_title'];
    ...
    //     echo "<pre>";
	   ...
	   // die();
		...
		)
		{
		  ...
		  //  print_r($data);
		  //  die();
			if(insert($data,'tbl_hospital_certificate'))
			{
	...
	else if(isset($_POST['btn_edit_cer']))
	{
		...
		)
		{
			...
			if(update($data,'tbl_hospital_certificate'))
			{
...
else if($_SERVER['REQUEST_METHOD'] == "GET")
{
	if(isset($_GET['act']) && $_GET['act'] == "del")
	{
		if(isset($_GET['certificate_id']) && $_GET['certificate_id'] != "" && $_GET['certificate_id'] != null && $_GET['certificate_id'] > 0 )
		{
			...
			where('certificate_id',$cer_id);
			if(delete('tbl_hospital_certificate'))
			{
