...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
	if(isset($_POST['action']) && $_POST['action'] == "getUserList")
	{
		...
		{
			$data = array();
			$sql = query("SELECT * FROM tbl_hospital WHERE hospital_id != ".get_sess("userdata")['hospital_id']);
			if(nrows($sql) > 0)
			{
				$i = 0;
				while($sqlData = fetch($sql))
	...
	else if(isset($_POST['txt_action']))
    {
        ...
        $data['sender']       	= get_sess("userdata")['hospital_id'];
        $data['sender_type']  	= "hospital";
    ...
    else if(isset($_POST['fetch_list']))
    {
        ...
        // print_r($list);
        // die();
        $doctor_id = get_sess("userdata")['hospital_id'];
    ...
    {
    	$receiver = get_sess("userdata")['hospital_id'];
...
else if($_SERVER['REQUEST_METHOD'] == "GET")
{
	...
	{
		$data['my_id'] 			= get_sess("userdata")['hospital_id'];
		$data['my_type'] 		= 'hospital';
		...
		if(insert($data,'tbl_user_contact'))
		{
            ...
            $myName = get_sess("userdata")['hospital_name'];
			$mySlug = get_sess("userdata")['hospital_slug'];
			$myImg 	= get_sess("userdata")['hospital_icon'];
			$message = '<li>
