...
if(isset($_GET['slug']) && $_GET['slug'] != "" && $_GET['slug'] != null)
{
    ...
    $sql = query("SELECT * FROM tbl_clinic WHERE clinic_active = 1 AND clinic_slug = '$slug'");
    if(nrows($sql) > 0)
    {
        $clinic             = fetch($sql);
        $clinicID           = $clinic['clinic_id'];
        $meta_title         = $clinic['clinic_meta_title'];
        $meta_title_ar      = $clinic['clinic_meta_title_ar'];
        $meta_keyword       = $clinic['clinic_meta_tag'];
        $meta_keyword_ar    = $clinic['clinic_meta_tag_ar'];
        $meta_desc          = $clinic['clinic_meta_desc'];
        $meta_desc_ar       = $clinic['clinic_meta_desc_ar'];
        ...
        ?>
            <header id="header" class="header">
			    <div class="wsmobileheader clearfix">
			    	...
			    	<span class="smllogo">
		    	    <img src="<?= file_url().$clinic['clinic_icon'];?>" style="width:180px;height:40px" alt="mobile-logo"/></span>
                ...
                <div class="wsmainfull menu clearfix">
    				<div class="wsmainwp clearfix" <?= ($lang == "eng") ? '' : 'style="direction:rtl !important"' ;?>>
    					<div class="desktoplogo" <?= ($lang == "eng") ? '' : 'style="float:right"' ;?>>
    					    <a href="#hero-10">
    					        <img src="<?= file_url().$clinic['clinic_icon'];?>"  style="width:180px;height:40px" alt="header-logo">
                            </a>
                        </div>
            ...
            <section id="doctor-top" class="doctor-details-section">
                <div class="container p-0" <?= ($lang == "eng") ? '' : 'style="direction:rtl !important;text-align:right"' ;?>>
                    <div class="row">
                        <div class="col-md-12">
                            <?php
                            $sql        = query("SELECT COUNT(slider_id) as count FROM tbl_clinic_slider WHERE slider_is_video = 1 AND slider_active = 1 AND slider_clinic = $clinicID ");
                            $is_video   = fetch($sql)['count'];
                            if($is_video > 0)
                            {
                                $sql = query("SELECT * FROM tbl_clinic_slider WHERE slider_video_show = 1 AND slider_active = 1 AND slider_clinic = $clinicID ");
                            ...
                            else
                            {
                                ...
                                <section id="hero-9" class="hero-section division">
                                	<div class="slider blue-nav">
                                    	<ul class="slides">
                                    	    <?php
                                    	    $sql = query("SELECT * FROM tbl_clinic_slider WHERE slider_is_video = 0 AND slider_active = 1 AND slider_clinic = $clinicID ");
                                    	    while($slData = fetch($sql))
                                    	    {
                                    	        ?>
                                    	        <li id="slide-1">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="appointmentDiv" id="doctors" style="margin:20px 0px 0px 0px;">
                                ...
                                <div class="row">
                                    <?php
                        		    $sql = query("SELECT * FROM tbl_clinic_doc WHERE doc_clinic = $clinicID AND doc_active = 1");
                        		    while($doc = fetch($sql))
                        		    {
                        		        ?>
                            ...
                            <div class="appointmentDiv" id="services" style="margin:20px 0px 0px 0px;">
                                ...
                                <div id="tabs-nav" class="list-group text-center">
                    			    <ul class="nav nav-pills" id="pills-tab" role="tablist" style="list-style:none !important">
                    			        <?php
                    			        $i = 0;
                    			        $sql = query("SELECT * FROM tbl_clinic_service WHERE dpt_clinic_id = $clinicID");
                    			        while($pkg = fetch($sql))
                    			        {
                    			            $i++;
                    			            ?>
                        ...
                        <div class="col-md-12">
                            <div class="appointmentDiv" id="offers" style="margin:20px 0px 0px 0px;<?= ($lang == "eng") ? '' : 'direction:rtl !important;text-align:right' ;?>">
                                ...
                                <div class="owl-carousel owl-theme offer-holder gallery clearfix">
                                    <?php
                                    $sql = query("SELECT * FROM tbl_clinic_offers WHERE offer_clinic = $clinicID");
                                    while ($row = fetch($sql))
                                    {
                                        ?>
                        ...
                        <div class="col-md-12">
                            <div class="appointmentDiv" id="photos" style="margin:20px 0px 0px 0px;<?= ($lang == "eng") ? '' : 'direction:rtl !important;text-align:right' ;?>">
                                ...
                                <div class="owl-carousel owl-theme images-holder gallery clearfix">
                				    <?php
                				    $sql = query("SELECT * FROM tbl_clinic_gallery WHERE gall_clinic = $clinicID");
                         		    while($dpt = fetch($sql))
                         		    {
                         		        ?>
                        ...
                        <div class="col-md-12">
                            <div class="appointmentDiv" id="videos" style="margin:20px 0px 0px 0px;<?= ($lang == "eng") ? '' : 'direction:rtl !important;text-align:right' ;?>">
                                ...
                                <div class="owl-carousel owl-theme video-holder">
                				    <?php
                				    $sql = query("SELECT * FROM tbl_clinic_video WHERE video_clinic = $clinicID");
                         		    while($dpt = fetch($sql))
                         		    {
                         		        ?>
                    					<div class="image-gall" <?= ($lang == "eng") ? '' : 'style="direction:rtl !important;text-align:right"' ;?>>
            		...
            		<div class="row">
            			<div class="col-md-12">					
            				<div class="owl-carousel owl-theme certificate-holder">
            				    <?php
            				    $sql = query("SELECT * FROM tbl_clinic_certificate WHERE certificate_active = 1 AND certificate_clinic = $clinicID ORDER BY certificate_id DESC");
                     		    while($dpt = fetch($sql))
                     		    {
                     		        ?>
            		...
            		<div class="row">
            			<div class="col-md-12">					
            				<div class="owl-carousel owl-theme reviews-holder">
            				    <?php
            				    $rSql = query("SELECT * FROM tbl_clinic_testimonial WHERE testimonial_clinic = $clinicID");
            		...
            		<div class="row">
            			<div class="col-md-12">					
            				<div class="owl-carousel owl-theme insurance-holder">
            				    <?php
            				    $sql = query("SELECT * FROM tbl_clinic_insurance WHERE insurance_active = 1 AND insurance_clinic = $clinicID ORDER BY insurance_id DESC");
                     		    while($dpt = fetch($sql))
                     		    {
                     		        ?>
            		...
            		<div class="row">
            			<div class="col-md-6 col-lg-3">
            				<div class="footer-info mb-40" <?= ($lang == "eng") ? '' : 'style="direction:rtl !important;text-align:right"' ;?>>
            					<a href="<?= base_url();?>"><img src="<?= file_url().$clinic['clinic_icon'];?>" width="180" height="40" alt="footer-logo"></a>
            					<p class="p-sm mt-20"><?= ($lang == "eng") ? $clinic['clinic_about'] : $clinic['clinic_about_ar']; ?>
            					</p>
            					<div class="footer-socials-links mt-20">
            						<ul class="foo-socials text-center clearfix">
            							<li><a target="_blank" href="<?= $clinic['clinic_facebook'];?>" class="ico-facebook"><i class="fab fa-facebook-f"></i></a></li>
            							<li><a target="_blank" href="<?= $clinic['clinic_twitter'];?>" class="ico-twitter"><i class="fab fa-twitter"></i></a></li>	
            							<li><a target="_blank" href="<?= $clinic['clinic_youtube'];?>" class="ico-google-plus"><i class="fab fa-youtube"></i></a></li>
            							<li><a target="_blank" href="<?= $clinic['clinic_linkedin'];?>" class="ico-linkedin-in"><i class="fab fa-linkedin-in"></i></a></li>
            							<li><a target="_blank" href="<?= $clinic['clinic_instagram'];?>" class="ico-linkedin-in"><i class="fab fa-instagram"></i></a></li>
            						</ul>									
            					</div>
            				</div>
            			</div>
            			<div class="col-md-6 col-lg-3">
            				<div class="footer-box mb-40" <?= ($lang == "eng") ? '' : 'style="direction:rtl !important;text-align:right"' ;?>>
            					...
            					<?php
                                $query = query("SELECT * FROM tbl_clinic_loc pc JOIN tbl_country c ON (c.country_id = pc.loc_country) JOIN tbl_cities ci ON (ci.city_id = pc.loc_city) WHERE loc_clinic_id = $clinicID");
                                ...
                                if($clinic['clinic_url'] != null && $clinic['clinic_url'] != ""){
                                    ?>
                                    <p><a style="color:white" href="<?= $clinic['clinic_url'];?>" target="_blank"><?= $clinic['clinic_url'];?></a></p>
                                    <?php
                                }
                                ?>
            				</div>
