...
{
    $query = $_GET['s'];
    $where .= " AND (cme_topic LIKE '%".$query."%' OR cme_ar_topic LIKE '%".$query."%' OR cme_des LIKE '%".$query."%' OR cme_ar_des LIKE '%".$query."%')";
}
if(isset($_GET['d']) && $_GET['d'] != "" && $_GET['d'] != null)
{
    ...
    {
        $query = $_GET['d'];
        $where .= " AND (cme_depart LIKE '%".$query."%' OR cme_ar_depart LIKE '%".$query."%') ";
    }
}
if(isset($_GET['l']) && $_GET['l'] != "" && $_GET['l'] != null)
{
    ...
    {
        $query = $_GET['l'];
        $where .= " AND (cme_loc LIKE '%".$query."%' OR cme_ar_loc LIKE '%".$query."%' )";
    }
}
...
$departsql = query("SELECT DISTINCT cme_depart, cme_ar_depart FROM tbl_cme ORDER BY cme_depart ASC ");
$locationsql = query("SELECT DISTINCT cme_loc, cme_ar_loc FROM tbl_cme ORDER BY cme_loc ASC ");
?>
<style>
.doctor-meta h6{
            ...
            <div class="row ">
    			...
    			<div class="col-sm-3">
    				...
    				<select name="d" class="form-control select">
                        ...
                        {
                            ?>
                            <option <?= (isset($_GET['d']) && ($_GET['d'] == $depart['cme_depart'] || $_GET['d'] == $depart['cme_ar_depart'])) ? 'selected' : '';?> value="<?= ($lang == "eng") ? $depart['cme_depart'] : $depart['cme_ar_depart']; ?>"><?= ($lang == "eng") ? $depart['cme_depart'] : $depart['cme_ar_depart']; ?></option>
                            <?php
                        }
                    	?>
                    </select>
                ...
                <div class="col-sm-3">
                    ...
                    <select name="l" class="form-control select">
                        ...
                        {
                            ?>
                            <option <?= (isset($_GET['l']) && ($_GET['l'] == $location['cme_loc'] || $_GET['l'] == $location['cme_ar_loc'])) ? 'selected' : '';?> value="<?= ($lang == "eng") ? $location['cme_loc'] : $location['cme_ar_loc']; ?>"><?= ($lang == "eng") ? $location['cme_loc'] : $location['cme_ar_loc']; ?></option>
                            <?php
                        }
                    	?>
                    </select>
...
<section id="doctors-3" class="bg-lightgrey wide-60 doctors-section division">
	<div class="container">
	    <div class="row">
            <?php
		    $sql = query("SELECT * FROM tbl_cme c LEFT JOIN tbl_organizer o on (c.cme_organizer = o.org_id) WHERE 1=1 $where LIMIT $offset, $limit");
		    $i=0;
		    while($cme = fetch($sql))
		    {
    				    ...
    				    <div class="row">
        			        <div class="col-md-6">
            					...
            					<div class="hover-overlay img-holder"> 
            						<img class="img-fluid" src="<?= file_url().$cme['cme_icon'];?>" alt="Job-Icon">	
            					</div>
        					</div>
        					<div class="col-md-6" style="border:none;">
        					    <div class="doctor-meta <?= ($lang == "eng") ? 'text-left' : 'text-right' ;?> " <?= ($lang == "eng") ? '' : 'style="direction:rtl !important;text-align:right"' ;?>>
        					        <h5 class="h5-xs blue-color" style="font-size: 16px !important;"><?= ($lang == "eng") ? $cme['cme_topic'] : $cme['cme_ar_topic']; ?></h5>
            						<h6><span><?= ($lang == "eng") ? $lang_con[57]['lang_eng'] : $lang_con[57]['lang_arabic']; ?></span> : <?= ($lang == "eng") ? $cme['cme_depart'] : $cme['cme_ar_depart']; ?></h6>
            						<h6><span><?= ($lang == "eng") ? $lang_con[175]['lang_eng'] : $lang_con[175]['lang_arabic']; ?></span> : <?= ($lang == "eng") ? $cme['cme_delivery'] : $cme['cme_ar_delivery']; ?></h6>
            						<h6><span><?= ($lang == "eng") ? $lang_con[175]['lang_eng'] : $lang_con[176]['lang_arabic']; ?> <?= ($lang == "eng") ? $lang_con[126]['lang_eng'] : $lang_con[126]['lang_arabic']; ?></span> : <?= ($lang == "eng") ? $cme['cme_date'] : $cme['cme_ar_date']; ?></h6>
            						<h6><span><?= ($lang == "eng") ? $lang_con[177]['lang_eng'] : $lang_con[177]['lang_arabic']; ?> <?= ($lang == "eng") ? $lang_con[126]['lang_eng'] : $lang_con[126]['lang_arabic']; ?></span> : <?= ($lang == "eng") ? $cme['close_date'] : $cme['close_date']; ?></h6>
            						<h6><span><?= ($lang == "eng") ? $lang_con[178]['lang_eng'] : $lang_con[178]['lang_arabic']; ?></span> : <?= ($lang == "eng") ? $cme['cme_time'] : $cme['cme_ar_time']; ?></h6>
            						<h6><span><?= ($lang == "eng") ? $lang_con[167]['lang_eng'] : $lang_con[167]['lang_arabic']; ?> <?= ($lang == "eng") ? $lang_con[179]['lang_eng'] : $lang_con[179]['lang_arabic']; ?></span> : <?= ($lang == "eng") ? $cme['cme_credits'] : $cme['cme_ar_credits']; ?></h6>
            						<a class="btn btn-sm btn-blue blue-hover mt-15" href="<?= base_url().$cme['cme_slug'];?>" title=""><?= ($lang == "eng") ? $lang_con[90]['lang_eng'] : $lang_con[90]['lang_arabic']; ?></a>
            					</div>
        					</div>
    					</div>
    				</div>
