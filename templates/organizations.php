				...
				<div class=" breadcrumb-holder">
					<nav aria-label="breadcrumb">
					  	<ol class="breadcrumb">
					    	...
					    	<li class="breadcrumb-item active" aria-current="page"><?= ($lang == "eng") ? $lang_con[225]['lang_eng'] : $lang_con[225]['lang_arabic']; ?></li>
					  	</ol>
					</nav>
					<h4 class="h4-sm steelblue-color"><?= ($lang == "eng") ? $lang_con[225]['lang_eng'] : $lang_con[225]['lang_arabic']; ?></h4>
				</div>
			</div>
		</div>
	</div>		
...
<section id="doctors-3" class="bg-lightgrey wide-60 doctors-section division">
	<div class="container">
		<div class="row">
		    <?php
		    $sql = query("SELECT * FROM tbl_organization WHERE organization_active = 1");
		    while($doc = fetch($sql))
		    {
		        ?>
    			<div class="col-md-6 col-lg-4">
    				<div class="doctor-2">
    					<div class="hover-overlay img-holder"> 
    						<img class="img-fluid" src="<?= file_url().$doc['organization_icon'];?>" alt="doctor-foto">	
    					</div>
    					<div class="doctor-meta">
    						<h5 class="h5-xs blue-color"><?= ($lang == "eng") ? $doc['organization_name'] : $doc['organization_name_ar']; ?></h5>
    						<span><?= ($lang == "eng") ? $doc['organization_phone'] : $doc['organization_phone_ar']; ?></span>
    						<a class="btn btn-sm btn-blue blue-hover mt-15" href="<?= base_url().$doc['organization_slug'];?>" title=""><?= ($lang == "eng") ? $lang_con[90]['lang_eng'] : $lang_con[90]['lang_arabic']; ?></a>
    					</div>
    				</div>
    			</div>
		        <?php
...
<?php 
$sql = query("SELECT * FROM tbl_advertisment WHERE add_location = 'organizations' AND add_status = 1 ORDER BY rand()");
if(nrows($sql) > 0)
{
?>
