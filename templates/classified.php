...
if(isset($_POST['d']) && $_POST['d'] != "" && $_POST['d'] != null)
{
    ...
    {
        $query = $_POST['d'];
        $where .= " AND (job_section LIKE '%".$query."%' OR job_section_ar LIKE '%".$query."%') ";
    }
}
...
$departsql = query("SELECT DISTINCT job_section, job_section_ar FROM tbl_classified_job ORDER BY job_section ASC ");
$locationsql = query("SELECT DISTINCT job_location, job_location_ar FROM tbl_classified_job ORDER BY job_location ASC ");
?>
<style>
.img-holder{
    padding-top:85%;
    ...
    <div class="container p-4 m-auto mb-4 no-gutters">
        <form method="post">
            <div class="row ">
    			...
    			<div class="col-sm-3">
    				<label class="form-label"><?= ($lang == "eng") ? $lang_con[205]['lang_eng'] : $lang_con[205]['lang_arabic']; ?>:</label>
    				<select name="d" class="form-control select">
                    	<option value="all"><?= ($lang == "eng") ? $lang_con[173]['lang_eng'] : $lang_con[173]['lang_arabic']; ?> <?= ($lang == "eng") ? $lang_con[205]['lang_eng'] : $lang_con[205]['lang_arabic']; ?></option>
                    	<?php
                    	while($depart = fetch($departsql))
                        {
                            ?>
                            <option <?= (isset($_POST['d']) && ($_POST['d'] == $depart['job_section'] || $_POST['d'] == $depart['job_section_ar'])) ? 'selected' : '';?> value="<?= ($lang == "eng") ? $depart['job_section'] : $depart['job_section_ar']; ?>"><?= ($lang == "eng") ? $depart['job_section'] : $depart['job_section_ar']; ?></option>
                            <?php
                        }
                    	?>
                    </select>
...
<section id="doctors-3" class="bg-lightgrey wide-60 doctors-section division">
	<div class="container">
	    <div class="row">
            <?php
		    $sql = query("SELECT * FROM tbl_classified_job WHERE 1=1 $where LIMIT $offset, $limit");
		    $i=0;
		    while($doc = fetch($sql))
		    {
		        if($i == 4)
    				    ...
    				    <div class="row">
        					...
        					<div class="col-md-6" style="border:none;">
        					    <div class="doctor-meta <?= ($lang == "eng") ? 'text-left' : 'text-right' ;?> " <?= ($lang == "eng") ? '' : 'style="direction:rtl !important;text-align:right"' ;?>>
            						...
            						<h6><?= ($lang == "eng") ? $doc['job_section'] : $doc['job_section_ar']; ?></h6>
        ...
        <div class="row d-flex justify-content-center mb-4">
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination">
                    <?php
				    $total_pages_sql = query("SELECT COUNT(*) as count FROM tbl_classified_job WHERE 1=1 $where");
