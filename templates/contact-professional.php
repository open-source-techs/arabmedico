...
if(isset($_GET['slug']) && $_GET['slug'] != "" && $_GET['slug'] != null)
{
    ...
    if(get_sess('employee_logged_in') == 1)
    {
        ...
        $emp_sql = query("SELECT * FROM tbl_employer where emp_id = '$emp_id'");
        $empData = fetch($emp_sql);
    }
    $slug = $_GET['slug'];
    include 'header.php';
        				...
        				<div class=" breadcrumb-holder">
        					<nav aria-label="breadcrumb">
        					  	<ol class="breadcrumb">
        					    	...
        					    	<li class="breadcrumb-item active" aria-current="page"><?= ($lang == "eng") ? $lang_con[183]['lang_eng'] : $lang_con[183]['lang_arabic']; ?> <?= ($lang == "eng") ? $lang_con[207]['lang_eng'] : $lang_con[207]['lang_arabic']; ?></li>
        					  	</ol>
        					</nav>
        					<h4 class="h4-sm steelblue-color"><?= ($lang == "eng") ? $lang_con[183]['lang_eng'] : $lang_con[183]['lang_arabic']; ?> <?= ($lang == "eng") ? $lang_con[207]['lang_eng'] : $lang_con[207]['lang_arabic']; ?></h4>
        				</div>
        			</div>
        		</div>
        	</div>
        	 		...
        	 		<div class="col-lg-8">
        	 			<div class="txt-block pr-30">
        					...
        					<div id="appointment-form-holder" class="text-center">
        					    <h5 class="h5-sm steelblue-color <?= ($lang == "eng") ? 'text-left' : 'text-right'; ?>"><?= ($lang == "eng") ? $lang_con[214]['lang_eng'] : $lang_con[214]['lang_arabic']; ?>: <span class="text-default"><?= ($lang == "eng") ? $doc['candidate_name'] : $doc['candidate_name_ar'];?></span></h5>
        					    <h5 class="h5-sm steelblue-color <?= ($lang == "eng") ? 'text-left' : 'text-right'; ?>"><?= ($lang == "eng") ? $lang_con[208]['lang_eng'] : $lang_con[208]['lang_arabic']; ?>: <span class="text-default"><?= ($lang == "eng") ? $doc['candidate_job'] : $doc['candidate_job_ar'];?></span></h5>
        						<form class="row job-form">
        			                <div id="input-name" class="col-lg-12 form-group m-2">
        			                    <input type="hidden" name="candiateid" value="<?= $doc['candidate_id'];?>">
        			                    <input type="hidden" name="employerid" value="<?= $emp_id;?>">
        			                    <input type="hidden" name="job" value="<?= $doc['candidate_job'];?>">
        			                    <input type="hidden" value="<?= ($lang == "eng") ? 'eng' : 'arab'; ?>" name="txt_lang">
        			                	<input type="text" name="name" class="form-control name" value="<?= ($lang == "eng") ? $empData['emp_name'] : $empData['emp_name_ar'];?>" placeholder="<?= ($lang == "eng") ? $lang_con[103]['lang_eng'] : $lang_con[103]['lang_arabic']; ?>*" required> 
        			                </div>
        			                <div id="input-email" class="col-lg-12 form-group m-2">
        			                	<input type="text" name="email" class="form-control email" value="<?= ($lang == "eng") ? $empData['emp_copEmail'] : $empData['emp_copEmail'];?>" placeholder="<?= ($lang == "eng") ? $lang_con[104]['lang_eng'] : $lang_con[104]['lang_arabic']; ?>*" required> 
        			                </div>
        			                <div id="input-phone" class="col-lg-12 form-group m-2">
        			                	<input type="tel" name="phone" class="form-control phone" value="<?= ($lang == "eng") ? $empData['emp_number'] : $empData['emp_number_ar']; ?>" placeholder="<?= ($lang == "eng") ? $lang_con[127]['lang_eng'] : $lang_con[127]['lang_arabic']; ?>*" required> 
        			                </div>
        			                <div id="input-message" class="col-lg-12 form-group m-2 <?= ($lang == "eng") ? 'text-left' : 'text-right'; ?>">
        			                    <textarea class="form-control" name="message" row="3" placeholder="Enter you message here..."></textarea>
        			                </div>
        			                <?php
        			                if(get_sess('employee_logged_in') == 1)
                                    {
                                        ...
                                        <div class="col-lg-12 form-btn">  
            			                	<button type="submit" name="btn_apply" class="btn btn-blue blue-hover submit"><?= ($lang == "eng") ? $lang_con[215]['lang_eng'] : $lang_con[215]['lang_arabic']; ?></button> 
            			                </div>
                                        <?php
                                    }
                                    else
                                    {
                                        ?>
                                        <p class="p-sm grey-color ml-30"><?= ($lang == "eng") ? $lang_con[217]['lang_eng'] : $lang_con[217]['lang_arabic']; ?></p>
                                        <?php
                                    }
        			                ?>
        ...
        <script>
            $(".job-form").validate({
    		    ...
    		    submitHandler: function(form){
    		        ...
    		        $.ajax({
                        type:'POST',
                        url: "<?= base_url();?>actions/send-message.php",
                        data: formData,
                        cache:false,
                        contentType: false,
                        processData: false,
