...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
	if(isset($_POST['action']) && $_POST['action'] == "getUserList")
	{
		...
		{
			$data = array();
			$sql = query("SELECT * FROM tbl_employer WHERE emp_id != ".get_sess("userdata")['emp_id']);
			if(nrows($sql) > 0)
			{
				$i = 0;
				while($sqlData = fetch($sql))
	...
	else if(isset($_POST['txt_action']))
    {
        ...
        $data['sender']       	= get_sess("userdata")['emp_id'];
        $data['sender_type']  	= "employer";
    ...
    else if(isset($_POST['fetch_list']))
    {
        ...
        $doctor_id = get_sess("userdata")['emp_id'];
    ...
    {
    	$receiver = get_sess("userdata")['emp_id'];
...
else if($_SERVER['REQUEST_METHOD'] == "GET")
{
	if(isset($_GET['act']) && $_GET['act'] == 'addContact')
	{
		...
		if(insert($data,'tbl_user_contact'))
		{
            ...
            $myName = get_sess("userdata")['emp_name'];
			$mySlug = get_sess("userdata")['emp_url'];
			$myImg 	= get_sess("userdata")['emp_logo'];
			$message = '<li>
                <div class="border-gray">
                    <div class="pull-left">
                        <img src="'.file_url().$myImg.'" class="img-thumbnail" alt="Employer Image">
                    </div>
                    <h4><a href="'.base_url().$mySlug.'" target="_blank">'.$myName.'</a> (Employer)</h4>
