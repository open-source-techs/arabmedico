    	                ...
    	                <div class="chat_list">
    	                    <ul class="user_list list-unstyled mb-0 mt-3 myclass">
    	                    		...
    	                    		<li <?= ($active) ? 'class="active"' : ''; ?> id="div<?= $senderID;?>">
    	                                <a href="<?= admin_base_url();?>inbox?IdChat=<?= $senderID;?>&Utype=<?= $senders['sender_type'];?>">
    	                                	...
    	                                	if($senders['sender_type'] == "employer")
    	                                	{
    	                                		...
    	                                		$docData 	= fetch($docSQl);
    	                                		?>
    	                                		<img src="<?= file_url().$docData['emp_icon']?>" alt="doctor-image" />
    		                                    <div class="about">
    	                        ...
    	                        if(isset($senderlist[0]))
    	                        {
    	                            ...
    	                            $sendersql = query("SELECT DISTINCT(c.receiver), c.receiver_type FROM tbl_chat c WHERE c.sender = $doctorID AND sender_type = 'doctor' AND c.receiver NOT IN ($send_list) ");
    	                        }
    	                        else
    	                        {
    	                            $sendersql = query("SELECT DISTINCT(c.receiver), c.receiver_type FROM tbl_chat c WHERE c.sender = $doctorID AND sender_type = 'doctor'");
    	                        }
