...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
	if(isset($_POST['btn_save_slide']))
	{
	    ...
	    $data['slider_org']         	= post('txt_org_id');
	    if($data['slider_is_video'] == 1)
	    {
	       ...
	       // print_r($data);
	       // die();
	        if(insert2($data, 'tbl_org_slider'))
			{
	...
	else if(isset($_POST['btn_edit_slide']))
	{
	    ...
	    if($data['slider_is_video'] == 1)
	    {
    		...
    		if(update2($data, 'tbl_org_slider'))
			{
				set_msg('Success','Slide is added successfully','success');
				jump(admin_base_url()."slider");
			}
	...
	else if(isset($_POST['action']) && $_POST['action'] == "change_status")
	{
	    ...
	    if($value == "deactive")
	    {
	        $sql = query("UPDATE tbl_org_slider SET slider_video_show = 0 WHERE slider_org = $clinicId");
	        if($sql)
	        {
	            $arr = array(
	                "status" => "done",
	    ...
	    else if($value == "active")
	    {
	        ...
	        if($sql)
	        {
	            ...
	            if(update($data1, 'tbl_org_slider'))
    			{
    				$arr = array(
    	                "status" => "done",
